...
- 가변인수(varargs) : 메서드에 넘기는 인수의 개수를 클라이언트가 조절할 수 있게 하는 기능
  - 배열을 생성해 인수를 전달

- 매개변수를 선언할 때 실체화 불가 타입으로 varargs 매개변수를 선언하면 컴파일러가 경고를 보낸다.

- 제네릭과 가변인수를 함께 쓰면 문제가 발생할 수 있다.
  - 타입 안전성이 깨질 수 있음

- @SafeVarargs : 제네릭 가변인수 메서드 작성자가 클라이언트  측에서 발생하는 경고를 숨길 수 있음
  - 메서드 작성자가 그 메서드가 타입 안전함을 보장하는 장치
  - 안전함이 확실하지 않다면 사용하지 않아야 한다.



- 제네릭 varargs 매개변수 배열에 다른 메서드가 접근하도록 허용하면 안전하지 않다.
  - 예외 1: @SafeVarargs로 제대로 선언된 또 다른 varargs 메서드에 넘기는 것
  - 예외 2: 이 배열 내용의 일부 함수를 호출만 하는 일반 메서드에 넘기는 것



- 제네릭이나 매개변수화 타입의 varargs 매개변수를 받는 모든 메서드에 @SafeVarargs를 달아야 함



- varargs를 List 매개변수로 직접 변환하여 사용하면 타입 안전하다.
